...
#if defined(CONFIG_RTC_ALARM_BOOT)
	int irq2;
	struct wake_lock alarmboot_wake_lock;
	struct delayed_work		alarmboot_work;
#endif
	int rtc_24hr_mode;
#if defined(RTC_SET_TIME_FILE_LOG)
...
//	spin_lock_irqsave(&info->rtc_set_time_lock, flags);
	...
	/* Read current time */
	max77686_rtc_read_time(dev, &cur_time);
...
#if defined(CONFIG_RTC_ALARM_BOOT)
static void max77693_alarmboot_work(struct work_struct *work)
{
	struct max77686_rtc_info *info = container_of(work,
						struct max77686_rtc_info, alarmboot_work.work);

	dev_info(info->dev, "%s:alarmboot_irq!\n", __func__);
...
#else
	if (lpcharge == 1) {
		if (!wake_lock_active(&info->alarmboot_wake_lock)) {
			wake_lock(&info->alarmboot_wake_lock);
			pr_debug("[%s](%d) alarmboot_wake_lock locked .\n",
				__func__, __LINE__);
		}
		schedule_delayed_work(&info->alarmboot_work, 2*HZ);
	}
#endif
...
#if defined(CONFIG_RTC_ALARM_BOOT)
	if (lpcharge == 1) {
		INIT_DELAYED_WORK_DEFERRABLE(&info->alarmboot_work,
							max77693_alarmboot_work);

		wake_lock_init(&info->alarmboot_wake_lock, WAKE_LOCK_SUSPEND,
								"alarmboot-wakelock");
	}
...
static int __devexit max77686_rtc_remove(struct platform_device *pdev)
{
	...
	if (info) {
		...
		if (lpcharge == 1) {
			if (wake_lock_active(&info->alarmboot_wake_lock)) {
				wake_unlock(&info->alarmboot_wake_lock);
				pr_err("[SPI] [%s](%d) spi_wakelock unlocked.\n",
					__func__, __LINE__);
			}
			wake_lock_destroy(&info->alarmboot_wake_lock);
			cancel_delayed_work(&info->alarmboot_work);
		}
